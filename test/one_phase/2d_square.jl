...
mp       = Particle2D{iInt, iFloat}(space_x=space_x, space_y=space_y, pos=mp_pos, ν=mp_ν, 
    E=mp_E, G=mp_G, σt=mp_σt, ϕ=mp_ϕ, c=mp_c, ψ=mp_ψ, ρs=mp_ρs, Ks=mp_Ks, NIC=init_NIC,
...
# visualization
@views @inbounds let
    figfont   = joinpath(assets, "fonts/cmu.ttf")
    font_size = 25
    fig       = Figure(font=figfont, fontsize=font_size)
    set_theme!(Theme(Axis=(xticklabelfont=figfont, yticklabelfont=figfont, 
        titlefont=figfont, xlabelfont=figfont, ylabelfont=figfont, aspect=DataAspect()), 
        Scatter=(strokewidth=0, markersize=20), Colorbar=(ticklabelfont=figfont, 
        spinewidth=0)))

    axis = Axis(fig[1, 1], xlabel=L"x", ylabel=L"y", aspect=DataAspect(), xticks=(0:2:8),
        yticks=(0:2:3))
    limits!(axis, -1, 8, -1, 3)
    scatter!(axis, mp.pos, color=mp.σm)
    display(fig)
