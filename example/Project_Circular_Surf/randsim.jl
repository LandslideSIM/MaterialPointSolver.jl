...
+------------------------------------------------------------------------------------------+
|  File Name  : randsim.jl                                                                 |
|  Description: 3D simple slope model with D-P. Random Field                               |
...
init_mp_in_space  = 2
init_project_name = "rand_circular_surf"
...
cp  = zeros(mp.num)
grf_gc!(cp, mp.pos, 1e3, 2e3)
ϕp  = zeros(mp.num)
grf_gc!(ϕp, mp.pos, 15*π/180, 30*π/180)
ψp  = fill!(zeros(mp.num), init_ψ)
σtp = fill!(zeros(mp.num), init_σt)
Gp  = fill!(zeros(mp.num), init_G)
Ksp = fill!(zeros(mp.num), init_Ks)
...
# MPM solver
testmaterialpointsolver!(args, grid, mp, pts_attr, bc, cp, ϕp, ψp, σtp, Gp, Ksp, workflow=testprocedure!)
