...
hdf5_name = "3d_slumping_vector_ep"
...
@views function processing_vector(hdf5_path::String, hdf5_name::String, sampling_step)
    ...
    paraview_collection(mps_path) do pvd
        @inbounds Threads.@threads for i in 1:itr
            ...
            y      = LinRange( 0, 20, 100)
            o      = (48, 10, 18)
            softid = findall(i->(mp_init[i, 3]≥((mp_init[i, 1]-o[1])^2/a^2+
                                                (mp_init[i, 2]-o[2])^2/b^2)/2+o[3])&&
                                (x[1]≤mp_init[i, 1]≤x[end])&&
                                (y[1]≤mp_init[i, 2]≤y[end]), 1:mp_num)
            lt = [minimum(mp_init[softid, 1]), minimum(mp_init[softid, 2])]
            rb = [maximum(mp_init[softid, 1]), maximum(mp_init[softid, 2])]
            ...
            p2c = fld.(mp_init[softid, 2].-gridyrange[1], sampling_step).*(length(gridxrange).-1).+
                  cld.(mp_init[softid, 1].-gridxrange[1], sampling_step) .|> Int
            cell_pts = zeros(cellnum, 5) # 1z, 2vx, 3vy, 4vz, 5mp_id
            cell_pts[:, 1] .= minimum(mp_init[softid, 3])
            for i in 1:length(softid)
                if p2c[i] > 0 
                    if cell_pts[p2c[i], 1] ≤ mp_init[softid[i], 3]
                        cell_pts[p2c[i], 2] = v_s[softid[i], 1]
                        cell_pts[p2c[i], 3] = v_s[softid[i], 2]
                        cell_pts[p2c[i], 4] = v_s[softid[i], 3]
                        cell_pts[p2c[i], 5] = softid[i]
                    end
                end
            end

            tmp = findall(all(!iszero, cell_pts[i, 5]) for i in 1:cellnum)
...
processing_vector(hdf5_path, hdf5_name, 1)