...
function run_mpm(comm, rank, gpu_counts, space_xyz, rst)
    ...
    args, grid, mp, pts_attr, bc = init_model(;
        ...
        init_grid_range_y   = round.([rank*12-init_grid_space_y*12, (rank+1)*12+init_grid_space_y*12], digits=4),
        init_grid_range_z   = round.([       -init_grid_space_z*3 ,          12+init_grid_space_z*3 ], digits=4),
        init_mp_range_x     = round.([      0+init_mp_offset      ,          60-init_mp_offset      ], digits=6),
        init_mp_range_y     = round.([rank*12+init_mp_offset      , (rank+1)*12-init_mp_offset      ], digits=6),
        init_mp_range_z     = round.([      0+init_mp_offset      ,          12-init_mp_offset      ], digits=6),
    ...
    if gpu_counts != 1
        if rank == 0
            halo_left  = (rank + 1) * 12 - space_xyz * 12
            halo_right = (rank + 1) * 12 + space_xyz * 12
            halo_tmp_idx = Int32.(findall(i -> halo_left ≤ grid.pos[i, 2] ≤ halo_right, 
                1:grid.node_num))
        ...
        elseif rank == gpu_counts-1
            halo_left  = rank * 12 - space_xyz * 12
            halo_right = rank * 12 + space_xyz * 12
            halo_tmp_idx = Int32.(findall(i -> halo_left ≤ grid.pos[i, 2] ≤ halo_right, 
                1:grid.node_num))
            tmp = hcat(grid.pos[halo_tmp_idx, :], collect(1:1:length(halo_tmp_idx)))
            sortid = sortslices(tmp, dims=1)[:, end] .|> Int32
            ...
            halo_left1  = rank * 12 - space_xyz * 12
            halo_right1 = rank * 12 + space_xyz * 12
            halo_tmp_idx1 = Int32.(findall(i -> halo_left1 ≤ grid.pos[i, 2] ≤ halo_right1, 
                1:grid.node_num))
            ...
            halo_left2  = (rank + 1) * 12 - space_xyz * 12
            halo_right2 = (rank + 1) * 12 + space_xyz * 12
            halo_tmp_idx2 = Int32.(findall(i -> halo_left2 ≤ grid.pos[i, 2] ≤ halo_right2, 
                1:grid.node_num))
    ...
    if gpu_counts != 1
        if rank == 0
            while Ti < args.Ttol
                ...
                testsolvegrid_OS!(dev)(ndrange=gpu_grid.node_num, gpu_grid, gpu_bc, args.ΔT, args.ζs)
                testdoublemapping1_OS!(dev)(ndrange=gpu_mp.num, gpu_grid, gpu_mp, gpu_pts_attr, 
                ...
                testdoublemapping3_OS!(dev)(ndrange=gpu_grid.node_num, gpu_grid, gpu_bc, args.ΔT)
        ...
        elseif rank == gpu_counts - 1
            ...
            while Ti < args.Ttol
                ...
                testsolvegrid_OS!(dev)(ndrange=grid.node_num, gpu_grid, gpu_bc, args.ΔT, args.ζs)
                testdoublemapping1_OS!(dev)(ndrange=mp.num, gpu_grid, gpu_mp, gpu_pts_attr, 
                    args.ΔT, args.FLIP, args.PIC)
                testdoublemapping2_OS!(dev)(ndrange=mp.num, gpu_grid, gpu_mp)
                ...
                testdoublemapping3_OS!(dev)(ndrange=grid.node_num, gpu_grid, gpu_bc, args.ΔT)
                testG2P_OS!(dev)(ndrange=mp.num, gpu_grid, gpu_mp)
                liE!(dev)(ndrange=mp.num, gpu_mp, gpu_pts_attr)
                if Ti≥args.Te
                    testdpP!(dev)(ndrange=mp.num, gpu_mp, gpu_pts_attr)
                end
                if args.MVL == true
