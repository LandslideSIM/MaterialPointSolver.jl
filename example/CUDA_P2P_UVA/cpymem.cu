...
int main() {
    ...
    // cudaSetDevice(1);
    // checkCudaStatus(cudaDeviceEnablePeerAccess(0, 0));

    // 记录复制开始的时间
    cudaSetDevice(0);
    cudaEventRecord(start);

    // 从GPU 1的数组B复制数据到GPU 0的数组A
    checkCudaStatus(cudaMemcpyPeer(A, 0, B, 1, size));
    ...
    std::cout << "Time for P2P copy: " << seconds << " seconds." << std::endl;
    //std::cout << "Time for P2P copy: " << milliseconds << " milliseconds." << std::endl;
