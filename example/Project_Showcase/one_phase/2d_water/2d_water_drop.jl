...
init_grid_range_y = [-0.1, 0.7]
init_mp_in_space  = 2
init_project_name = "2d_water_drop"
...
init_ρs           = 1e3
init_ν            = 0
init_Ks           = 2e8 # smaller than real value for smaller time step
init_E            = 1e6
init_G            = init_E/(2*(1+init_ν))
init_T            = 2
init_Te           = 0
init_ΔT           = 0.1*min(init_grid_space_x, init_grid_space_y)/sqrt(init_Ks/init_ρs)
...
range_x  = [0.1+grid.space_x/init_mp_in_space/2, 0.3-grid.space_x/init_mp_in_space/2]
range_y  = [0.3+grid.space_y/init_mp_in_space/2, 0.5-grid.space_y/init_mp_in_space/2]
...
tmp_idx = findall(i->(grid.pos[i, 1]≤0)||(grid.pos[i, 1]≥0.6), 1:grid.node_num)
tmp_idy = findall(i->(grid.pos[i, 2]≤0)||(grid.pos[i, 2]≥0.6), 1:grid.node_num)
vx_idx[tmp_idx] .= 1
vy_idx[tmp_idy] .= 1
